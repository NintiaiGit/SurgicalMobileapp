...
public class OperatorListActivity extends AppCompatActivity {
     ...
     * @return OperatorListActivity Intent.
     * since 3.5.6
     */
    @NonNull
    public static Intent newIntent(@NonNull Context context, @NonNull String channelUrl, @StyleRes int themeResId) {
        return newIntentFromCustomActivity(context, OperatorListActivity.class, channelUrl, themeResId);
    }
    ...
    /**
     * Create an intent for a custom activity. The custom activity must inherit {@link OperatorListActivity}.
     *
     * @param context    A Context of the application package implementing this class.
     * @param cls        The activity class that is to be used for the intent.
     * @param channelUrl the url of the channel will be implemented.
     ...
     */
    @NonNull
    public static Intent newIntentFromCustomActivity(@NonNull Context context, @NonNull Class<? extends OperatorListActivity> cls, @NonNull String channelUrl, @StyleRes int themeResId) {
    ...
    /**
     * It will be called when the {@link OperatorListActivity} is being created.
     * The data contained in Intent is delivered to Fragment's Bundle.
     *
     * @return {@link com.sendbird.uikit.fragments.OperatorListFragment}
     * since 3.0.0
     */
    @NonNull
    protected Fragment createFragment() {
        final Bundle args = getIntent() != null && getIntent().getExtras() != null ? getIntent().getExtras() : new Bundle();
        return SendbirdUIKit.getFragmentFactory().newOperatorListFragment(args.getString(StringSet.KEY_CHANNEL_URL, ""), args);
    }
}
